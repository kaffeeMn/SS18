...
Da nun $xz$ eine ungerade Anzahl an $a$ enth"alt, gibt es auch keine Aufteilung
    von $xz = u'a^i$, sodass gelten w"urde $\#_a(u') = a^i$.\\
Daraus folgt: $wz \not \in L_2$\\
\\
...
\\
Das Akzeptieren erfolgt "uber das Sequenzzielle durchlaufen aller Zeichen f"ur
    jedes gesuchte Wort.\\
...
\automaton{
    ...
    \path
        ...
        (maxim)
            ...
            edge [->, left, bend right] node {a} (ma)
        ...
        (ieren)
            edge [->, loop above] node {$\Sigma$} (ieren)
    ;}\\
Der Aufbau des DFA's erfolgt wie auch bei 4.2.1. Jedoch wurden noch transitionen
    , die f"ur den Determinismus notwendendig sind erg"anzt.\\
...
Wurde zuvor ein $m$ eingelesen, so kann wieder mit den Pr"afixen ma und mi 
    angefangen werden.
    Daher die Transitionen 
    $maxim \rao{a} ma, minim \rao{a} ma, minim \rao{i} mi$.\\
F"ur den Zustand $maxim$ wird keine Transition in $mi$ angef"ugt, da $mi$ nicht 
    akzeptierend ist und ein lauf zu $ieren$ noch m"oglich ist.
    Daf"ur existiert die Transition $i \rao{n} min$, die nachtr"aglich zum 
    Pr"afix min wechselt, da wir uns an der Stelle sicher sein k"onnen, dass
    kein anderer Pr"afix mehr m"oglich ist.\\
