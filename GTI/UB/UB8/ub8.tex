...
\automaton{
    ...
    ;}\\
\gap
Es wird davon ausgegangen, dass die W"orter unter korrektar Syntax eingegeben
    wurden.\\
Alternativ kann vor der gegebenen TM auch eine TM durchlaufen werden, die nach
    der Syntax pr"uft, so eine w"are in etwa:\\
\automaton{
    \nd{initial, state}{s}  {0,0}{$s$}
    \nd{state}{u}           {0,0}{$u$}
    \nd{state}{compare}     {0,0}{$compare$}
    \nd{state}{v}           {0,0}{$v$}
    \nd{state}{undo}        {0,0}{$undo$}
    \nd{state}{ja}          {0,0}{$ja$}
    ...
    \path
        (s)
            edge [->, loop above] node {$\begin{array}{l}
                                            ...
                                            \u{b} : \u{b}, \ra\\
                                        \end{array}$} (s')
            edge [->, right] node {}(compare)
            
            
    ;}
$s$ aus $M_1$ w"urde aso unmittelbar auf $ja$ aus $M_2$ folgen.\\
...
Die resultierende TM w"ahre damit:\\
