...
Es wurde nichts zu einer Datei gesagt, die als einziges Teilwort vor der 
    ...
    auch eine versteckte Datei mit Namen `tar' ohne Dateiendung sein.\\
(ii) Ferner muss auf mindestens eine Dateiendung aus der Menge 
    $\{.tar, .tar.gz, .tgz\}$ geendet werden. Wir nehmen an,
    dass gueltige Dateiendungen aufeinander folgen d"urfen, da mehrfaches 
    Vepacken m"oglich w"are.\\
\\
Konvention:\\
Hinsichtlich der "Ubersichtlichkeit werden Transitionen, wie auch in der 
    Vorlesung, die in den Senkenzustand f"uhren weggelassen. Falls von einem 
    Zustand aus also die Eingabe
    eines Zeichens nicht ber"ucksichtigt wird, ist das gleichbedeutend mit einer
    Transition in den Senkenzustand.\\
\automaton{
    \nd{initial, state}{s}          {0,-4}{}
    \nd{state}{char}                {0,-2} {}
    \nd{state}{v_}                  {0,0} {}
    \nd{state}{v_t}                 {2,0} {}
    \nd{state}{v_ta}                {4,0} {}
    \nd{accepting, state}{v_tar}    {6,0} {}
    \nd{state}{v_tar_}              {8,0} {}
    \nd{state}{v_tar_g}             {10,0} {}
    \nd{accepting, state}{v_tar_gz} {12,0} {}
    \path
        % verwerfen, bei leerem Wort
        (s)
            edge [->, left] node {a, ..., z} (char)
        ...
        (v_)
            edge [->, above] node {t} (v_t)
            edge [->, above] node {$\eps$} (char)
            %edge [->, left] node {$\eps$} (decl)
        (v_t)
            edge [->, above] node {a} (v_ta)
            edge [->, bend left, above] node {$\eps$} (v_tar_)
            edge [->, above] node {$\eps$} (char)
        (v_ta)
            edge [->, above] node {r} (v_tar)
            edge [->, above] node {$\eps$} (char)
...
\correction
Die $\eps$-closures aller Knoten erbegen sich zu:\\$
    \eps\text{-clousre}(1) = \{1,3,2\} 
        \text{, mit } 1 \rao{\eps} 3 \rao{\eps} 2\\
    \eps\text{-clousre}(2) = \{2\}
        \text{, mit } \nexists q \in Q : 2 \rao{\eps} q\\
    \eps\text{-clousre}(3) = \{3,2\}
        \text{, mit } 3 \rao{\eps} 2\\
    \eps\text{-clousre}(4) = \{4\}
        \text{, mit } \nexists q \in Q : 4 \rao{\eps} q\\
    $\\
Der folgende Automat $A$:\\
\automaton{
    \nd{initial, accepting, state}{123} {0,0}  {$\{1,2,3\}$}
    \nd{accepting, state}   {24}        {2,2}  {$\{2,4\}$}
    \nd{accepting, state}   {4}         {4,0}  {$\{4\}$}
    \nd{accepting, state}   {23}        {2,-2} {$\{2,3\}$}
    \nd{state}              {empty}     {6,0}  {$\emptyset$}
    \path
        (123)
            edge [->, above] node {a} (24)
            edge [->, right] node {b} (23)
...
% Korrigiert
\correction
Uns interessieren nur der Ausdruck $\alpha_{1,2}^{2}$, da 2
    der einzige Akzeptierende Zustand ist und wir in 1 starten.\\
wir erhalten $\alpha_{acc} = \alpha_{2,2}^{2} + \alpha_{1,2}^{2}$ mit:\\
\\$
    \alpha_{1,1}^{0} = a + c + \eps\\
    \alpha_{2,2}^{0} = a + b + \eps\\
    \alpha_{1,2}^{0} = b\\
    \alpha_{2,1}^{0} = c\\
    \gap
    \alpha_{1,1}^{1} 
        = \alpha_{1,1}^{0}
            + \alpha_{1,1}^{0}(\alpha_{1,1}^{0})^* \alpha_{1,1}^{0}
        \equiv (a+c+\eps) + (a+c+\eps)(a+c+\eps)^*(a+c+\eps) 
        \equiv (a+c)^*\\
    \alpha_{2,2}^{1}
        = \alpha_{2,2}^{0} 
            + \alpha_{2,1}^{0}(\alpha_{1,1}^{0})^* \alpha_{1,2}^{0}
        \equiv (a+b+\eps) + c(a+c+\eps)^*b\\
    \alpha_{1,2}^{1}
        = \alpha_{1,2}^{0} 
            + \alpha_{1,1}^{0}(\alpha_{1,1}^{0})^* \alpha_{1,2}^{0}
        \equiv b + (a+c+\eps) (a+c+\eps)^* b
        \equiv (a+c)^* b\\
    \alpha_{2,1}^{1}
        = \alpha_{2,1}^{0} 
            + \alpha_{2,1}^{0}(\alpha_{1,1}^{0})^* \alpha_{1,1}^{0}
        \equiv c + c (a+c+\eps)^* (a+c+\eps)
        \equiv c (a+c)^*\\
    \gap
    \alpha_{1,2}^{2}
        = \alpha_{1,2}^{1}
            + \alpha_{1,2}^{1}(\alpha_{2,2}^{1})^* \alpha_{2,2}^{1}\\
        \equiv ((a+c)^*b) 
            + ((a+c)^*b) ((a+b+\eps) + c(a+c+\eps)^*b)^* ((a+b+\eps) + c(a+c+\eps)^*b)\\
        \equiv (a+c)^*b ((a+b+\eps) + c(a+c+\eps)^*b)^*\\
    $\\
zu:\[
    \alpha_{acc} \mdef \alpha_{1,2}^2
        = (a+c)^*b ((a+b) + c(a+c)^*b)^*\\
    \]
% A3
\newpage
