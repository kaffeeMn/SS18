...
\begin{tikzpicture}
    ...
    \node[accepting, state](2)  at (2,2)  {$\{2\}$};
    \node[accepting, state](4)  at (4,1)  {$\{4\}$};
    \node[accepting, state](23) at (2,-2) {$\{2,3\}$};
    \node[accepting, state](24) at (4,-1) {$\{2,4\}$};
    ...
    \path
        ...
        (2)
            edge [->, above] node {a} (4)
            edge [->, bend left, above] node {b} (empty)
        (4)
            edge [->, above] node {a} (empty)
            edge [->, bend right, left] node {b} (23)
        (23)
            edge [->, above] node {a} (24)
            edge [->, bend right, below] node {b} (empty)
        (24)
            edge [->, left] node {a} (4)
            edge [->, bend right, left] node {b} (23)
        (empty)
...
\
\subsubsection\
Uns interessiert nur die Ausdr"ucke $\alpha_{2,2}^{2}, \alpha_{1,2}^{2}$, da 2
...
Sind korrekt, da:\\
\gap
Ferner gilt $L = (W_0 \setminus W_1) \setminus W_2$\\
Offensichtlich gilt $(W_0 \setminus W_1) = W_0$. $W_0 \setminus W_2$ ergibt sich zu:\\
\[
    \{w \in \{a,b\}^* | |w| \text{ gerade}\}
    \setminus \{w \in \Sigma^* | \#_c(w) > 0\}
