...
\\
Die Menge wurde gebildet, indem zuerst alle Variablen, die eine 
    Regel mit nur Terminalsymbolen enthalten hinzugef"ugt. Darauf folgend werden
    solange Variablen, die Regeln mit nur Variablen aus der Menge $V_e$ 
    enthalten, hinzugef"ugt, bis keine mehr gefunden werden.\\
\\
...
\\
Dies kann z.B. durch BFS oder DFS ausgehend vom Startsymbol S im Graphen
    ermittelt werden.\\
\\
$A$ und dessen Regeln werden entfernt.\\
\gap
...
Dabei werden die rechten Seiten so verk"urzt, dass die Bedingung gilt, dass
    ...
    Dies geschieht durch einf"ugen von Variablen, die Konkatenationen durch 
    Regeln ersetzen unter Aufrecherhaltung der Bedingung.\\
...
Anschlie"send werden alle regeln mit $\eps$ entfernt und f"ur jede Regel mit
    Variablen aus $V'$ auf der rechten Seite neue Regeln ohne diese Variablen
    angef"ugt.\\
\\
CNF5:\\
...
Wir betrachten die Aussage:\[
    \forall k \in \mathbb{N}, w \in L(G) \text{, mit w ist durch eine Ableitung
        der L"ange $k$ aus $G$ abgeleitet} 
        : w = a^nb^m , (m,n \in \mathbb{N}, m \geq 2n)
    \]
\\
Wir f"uhren einen Induktionsbeweis "uber die L"ange der Ableitung(sschritte) 
    k.\\
\gap
I.A.\\
\\
$k = 1$\\
Es existiert genau eine Ableitung der L"ange $1$ von $G$ mit:\[
    S \RA_G abb
    \], damit muss keine Fallunterscheidung betrachtet werden.\\
damit muss $w$ gew"ahlt werden mit $w \mdef abb$\\
es gilt:$\\
    \#_a(w) = 1\\
    \#_b(w) = 2 \geq 2 * 1 = 2 * \#_a(w)\\
    $
Damit gilt die Aussage f"ur k=1\\
\\
I.V.\\
\\
...
$k = k' + 1$\\
\\
Die Einzige Regel, die einen vorherigen Ableitungsschritt erlaubt ist:\[
    S \rA aSbb
    \], damit muss keine Fallunterscheidung betrachtet werden.\\
Wir betrachten nun das abgeleitete Wort $w \mdef avbb$, mit $v$ als ein Wort,
...
w \in L, w \not\in L(G) \lightning\\
$\\
w ist in $L$, da $w \equiv a^nb^m$, mit $n = 1, m = 3, m \geq 2n$ gilt.\\
w ist nicht in $L(G)$, da $G$ $abbb$ nicht ableiten kann.\\
Offensichtlich ist die Wortl"ange bei steigender Ableitungsl"ange monoton
    steigend.\\
\\
Ableitung der L"ange 1:\\$
    S \RA_G abb\\
    abbb \not \equiv abb, |abbb| > |abb|
    $\\
\\
Ableitung der L"ange 2:\\$
    S \RA_G aSbb\\
      \RA_G aabbbb\\
    abbb \not \equiv aabbbb, |abbb| < |aabbbb|
    $\\
\\
Damit gilt $abbb \not \in L(G)$\\
\\
